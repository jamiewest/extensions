...
class DiskBasedResultStore implements EvaluationResultStore {
  ...
  DiskBasedResultStore(String storageRootPath)
    : _resultsRootPath = _join(
        Directory(storageRootPath).absolute.path,
        'results',
      );
  ...
  Stream<ScenarioRunResult> readResults({
    ...
    for (final execDirPath in _enumerateExecutionDirs(
      resultsDir,
      executionName,
    )) {
      for (final scenDirPath in _enumerateScenarioDirs(
        execDirPath,
        scenarioName,
      )) {
        for (final filePath in _enumerateResultFiles(
          scenDirPath,
          iterationName,
        )) {
  ...
  static ScenarioRunResult _resultFromJson(Map<String, dynamic> j) =>
      ScenarioRunResult(
        ...
        messages: (j['messages'] as List)
            ...
            .toList(),
        modelResponse: _responseFromJson(
          j['modelResponse'] as Map<String, dynamic>,
        ),
        evaluationResult: _evalResultFromJson(
          j['evaluationResult'] as Map<String, dynamic>,
        ),
  ...
  static EvaluationResult _evalResultFromJson(Map<String, dynamic> j) {
    final map = (j['metrics'] as Map<String, dynamic>).map(
      (k, v) => MapEntry(k, _metricFromJson(v as Map<String, dynamic>)),
    );
  ...
  static EvaluationMetric _metricFromJson(Map<String, dynamic> j) {
    ...
    if (j['interpretation'] != null) {
      metric.interpretation = _interpretationFromJson(
        j['interpretation'] as Map<String, dynamic>,
      );
    }
