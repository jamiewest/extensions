...
/// An [Evaluator] that evaluates an AI system's performance in retrieving
/// information for additional context in response to a user request (for
/// example, in a Retrieval Augmented Generation (RAG) scenario).
///
...
/// It returns a [NumericMetric] that contains a score for 'Retrieval'. The
/// score is a number between 1 and 5, with 1 indicating a poor score, and 5
/// indicating an excellent score. High retrieval scores indicate that the AI
/// system has successfully extracted and ranked the most relevant information
/// at the top, without introducing bias from external knowledge and ignoring
/// factual correctness. Conversely, low retrieval scores suggest that the AI
/// system has failed to surface the most relevant context chunks at the top
/// of the list and / or introduced bias and ignored factual correctness.
/// Note: [RetrievalEvaluator] is an AI-based evaluator that uses an AI model
...
/// used. The prompt that [RetrievalEvaluator] uses has been tested against
...
/// GPT-4o
class RetrievalEvaluator implements Evaluator {
  RetrievalEvaluator();

  final ReadOnlyCollection<String> evaluationMetricNames = [RetrievalMetricName];

  static final ChatOptions _chatOptions;

  /// Gets the [Name] of the [NumericMetric] returned by [RetrievalEvaluator].
  static String get retrievalMetricName {
    return "Retrieval";
  }
  ...
  Future<EvaluationResult> evaluate(
    ...
    var metric = numericMetric(retrievalMetricName);
    ...
    if (additionalContext?.ofType<RetrievalEvaluatorContext>().firstOrDefault() is! RetrievalEvaluatorContext context) {
      metric.addDiagnostics(
                EvaluationDiagnostic.error(
    ...
    var evaluationInstructions = getEvaluationInstructions(userRequest, context);
  ...
  static List<ChatMessage> getEvaluationInstructions(
    ...
    for (final retrievedChunk in context.retrievedContextChunks) {
    ...
    _ = builder.append(']');
    var renderedContext = builder.toString();
    var evaluationPrompt = $''"
            ...
            **Examples:**
              ...
              **Context:** ["5. Cancel. Rappelling is the process of coming down from a mountain that is usually done with two pieces of rope. Use a natural anchor or a set of bolts to rappel from with help from an experienced rock climber in this free video on rappelling techniques. Part of the Video Series: Rappelling & Rock Climbing.", "abseiling(
                /ˈaebseɪl/ ˈæbseɪl /or/ ; ˈɑːpzaɪl From german,
                abseilen meaning to rope,
              ), down also called, rappelling is the controlled descent of a vertical, drop such as a rock, face using a. Rope climbers use this technique when a cliff or slope is too steep/and or dangerous to descend without. protection", "1. rappel - (mountaineering) a descent of a vertical cliff or wall made by using a doubled rope that is fixed to a higher point and wrapped around the body. abseil. mountain climbing, mountaineering-the activity of climbing a mountain. descent-the act of changing your location in a downward direction."]
            ...
            **Examples:**
            ...
            ## [Retrieval: 4] (
              Relevant Context Ranked Middle,
              No External Knowledge Bias and Factual Accuracy Ignored,
            )
