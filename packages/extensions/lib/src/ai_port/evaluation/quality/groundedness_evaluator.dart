...
/// An [Evaluator] that evaluates the 'Groundedness' of a response produced by
/// an AI model.
///
...
/// [GroundingContext]. It returns a [NumericMetric] that contains a score for
/// the 'Groundedness'. The score is a number between 1 and 5, with 1
/// indicating a poor score, and 5 indicating an excellent score. Note:
/// [GroundednessEvaluator] is an AI-based evaluator that uses an AI model to
...
/// used. The prompt that [GroundednessEvaluator] uses has been tested against
...
/// GPT-4o
class GroundednessEvaluator implements Evaluator {
  GroundednessEvaluator();

  final ReadOnlyCollection<String> evaluationMetricNames = [GroundednessMetricName];

  static final ChatOptions _chatOptions;

  /// Gets the [Name] of the [NumericMetric] returned by
  /// [GroundednessEvaluator].
  static String get groundednessMetricName {
    return "Groundedness";
  }
  ...
  Future<EvaluationResult> evaluate(
    ...
    var metric = numericMetric(groundednessMetricName);
    ...
    if (additionalContext?.ofType<GroundednessEvaluatorContext>().firstOrDefault() is! GroundednessEvaluatorContext context) {
      metric.addDiagnostics(
  ...
  static List<ChatMessage> getEvaluationInstructions(
    ...
    var groundingContext = context.groundingContext;
