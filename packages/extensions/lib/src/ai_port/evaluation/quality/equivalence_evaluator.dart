...
/// An [Evaluator] that evaluates the 'Equivalence' of a response produced by
/// an AI model with another response supplied via [GroundTruth].
///
/// Remarks: [EquivalenceEvaluator] measures the degree to which the response
/// being evaluated is similar to the response supplied via [GroundTruth]. It
/// returns a [NumericMetric] that contains a score for the 'Equivalence'. The
/// score is a number between 1 and 5, with 1 indicating a poor score, and 5
/// indicating an excellent score. Note: [EquivalenceEvaluator] is an AI-based
...
/// [EquivalenceEvaluator] uses has been tested against (and tuned to work
/// well with) the following models. So, using this evaluator with a model
/// from the following list is likely to produce the best results. (The model
/// to be used can be configured via [ChatClient].) GPT-4o
class EquivalenceEvaluator implements Evaluator {
  EquivalenceEvaluator();

  final ReadOnlyCollection<String> evaluationMetricNames = [EquivalenceMetricName];

  static final ChatOptions _chatOptions;

  /// Gets the [Name] of the [NumericMetric] returned by [EquivalenceEvaluator].
  static String get equivalenceMetricName {
    return "Equivalence";
  }
  ...
  Future<EvaluationResult> evaluate(
    ...
    var metric = numericMetric(equivalenceMetricName);
    ...
    if (additionalContext?.ofType<EquivalenceEvaluatorContext>().firstOrDefault() is! EquivalenceEvaluatorContext context) {
      metric.addDiagnostics(
    ...
    var evaluationInstructions = getEvaluationInstructions(userRequest, modelResponse, context);
  ...
  static List<ChatMessage> getEvaluationInstructions(
    ...
    var renderedUserRequest = userRequest?.renderText() ?? string.empty;
    ...
    var evaluationPrompt = $''"
            ...
            One star: the predicted answer is! at all similar to the correct answer
