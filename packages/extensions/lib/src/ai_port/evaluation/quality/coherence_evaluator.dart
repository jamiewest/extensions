...
/// An [Evaluator] that evaluates the 'Coherence' of a response produced by an
/// AI model.
///
/// Remarks: [CoherenceEvaluator] measures the readability and
/// user-friendliness of the response being evaluated. It assesses the ability
/// of an AI system to generate text that reads naturally, flows smoothly, and
/// resembles human-like language in its responses. [CoherenceEvaluator]
/// returns a [NumericMetric] that contains a score for 'Coherence'. The score
/// is a number between 1 and 5, with 1 indicating a poor score, and 5
/// indicating an excellent score. Note: [CoherenceEvaluator] is an AI-based
...
/// [CoherenceEvaluator] uses has been tested against (and tuned to work well
/// with) the following models. So, using this evaluator with a model from the
/// following list is likely to produce the best results. (The model to be
/// used can be configured via [ChatClient].) GPT-4o
class CoherenceEvaluator implements Evaluator {
  CoherenceEvaluator();

  final ReadOnlyCollection<String> evaluationMetricNames = [CoherenceMetricName];

  static final ChatOptions _chatOptions;

  /// Gets the [Name] of the [NumericMetric] returned by [CoherenceEvaluator].
  static String get coherenceMetricName {
    return "Coherence";
  }
  ...
  Future<EvaluationResult> evaluate(
    ...
    var metric = numericMetric(coherenceMetricName);
