...
/// An [Evaluator] that evaluates the 'Relevance', 'Truth' and 'Completeness'
/// of a response produced by an AI model.
///
/// Remarks: [RelevanceTruthAndCompletenessEvaluator] returns three
/// [NumericMetric]s that contain scores for 'Relevance (RTC)', 'Truth (RTC)'
/// and 'Completeness (RTC)' respectively. Each score is a number between 1
/// and 5, with 1 indicating a poor score, and 5 indicating an excellent
/// score. Note: [RelevanceTruthAndCompletenessEvaluator] is an AI-based
/// evaluator that uses an AI model to perform its evaluation. While the
/// prompt that this evaluator uses to perform its evaluation is designed to
/// be model-agnostic, the performance of this prompt (and the resulting
...
/// [RelevanceTruthAndCompletenessEvaluator] uses has been tested against (and
/// tuned to work well with) the following models. So, using this evaluator
/// with a model from the following list is likely to produce the best
/// results. (The model to be used can be configured via [ChatClient].) GPT-4o
class RelevanceTruthAndCompletenessEvaluator implements Evaluator {
  RelevanceTruthAndCompletenessEvaluator();

  final ReadOnlyCollection<String> evaluationMetricNames = [RelevanceMetricName, TruthMetricName, CompletenessMetricName];

  final ChatOptions _chatOptions;

  /// Gets the [Name] of the [NumericMetric] returned by
  /// [RelevanceTruthAndCompletenessEvaluator] for 'Relevance'.
  static String get relevanceMetricName {
    return "relevance(RTC)";
  }

  /// Gets the [Name] of the [NumericMetric] returned by
  /// [RelevanceTruthAndCompletenessEvaluator] for 'Truth'.
  static String get truthMetricName {
    return "truth(RTC)";
  }

  /// Gets the [Name] of the [NumericMetric] returned by
  /// [RelevanceTruthAndCompletenessEvaluator] for 'Completeness'.
  static String get completenessMetricName {
    return "completeness(RTC)";
  }
  ...
  Future<EvaluationResult> evaluate(
    ...
    var relevance = numericMetric(relevanceMetricName);
    var truth = numericMetric(truthMetricName);
    var completeness = numericMetric(completenessMetricName);
    var result = evaluationResult(relevance, truth, completeness);
  ...
  static List<ChatMessage> getEvaluationInstructions(
    ...
    ChatResponse modelResponse,
    Iterable<ChatMessage> conversationHistory,
  ) {
    var renderedUserRequest = userRequest.renderText();
    var renderedModelResponse = modelResponse.renderText();
    var renderedConversationHistory = conversationHistory.renderText();
    var evaluationPrompt = $''"
            ...
            - completeness_reason_lazy_unopinionated. The model claims not to know how, or not be certain enough, to address the user intent and does not offer the user any recourse (
              e.g.,
              asking the user to be more specific,
              or offering potential subtopics for ambiguous user requests,
            ) . Example: User: "compile error" Model response: "I can't help with that, I need more information." (The response doesn't offer any typical troubleshooting ideas based on the user's code, context, or general programming knowledge.)
            ...
            -----
            """;
    return [chatMessage(ChatRole.user, evaluationPrompt)];
  }
