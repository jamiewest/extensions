...
import '../../c_sharp/reporting_configuration.dart';
import '../../c_sharp/scenario_run.dart';
import '../../c_sharp/scenario_run_result.dart';
import 'azure_storage_response_cache_provider.dart';
import 'azure_storage_result_store.dart';

/// Contains factory method for creating a [ReportingConfiguration] that
/// persists [ScenarioRunResult]s to Azure Storage and also uses the storage
/// to cache AI responses.
class AzureStorageReportingConfiguration {
  AzureStorageReportingConfiguration();

  /// Creates a [ReportingConfiguration] that persists [ScenarioRunResult]s to
  /// Azure Storage and also uses the storage to cache AI responses.
  ///
  ...
  /// Azure Storage and also uses Azure Storage to cache AI responses.
  ///
  /// [client] A [DataLakeDirectoryClient] with access to an Azure Storage
  /// container under which the [ScenarioRunResult]s and all cached AI responses
  /// should be stored.
  ///
  ...
  static ReportingConfiguration create(
    ...
    var responseCacheProvider =
        ...
        ? azureStorageResponseCacheProvider(client, timeToLiveForCacheEntries)
        : null;
    var resultStore = azureStorageResultStore(client);
