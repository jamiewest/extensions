...
import '../common/string.dart' as string;

import 'instrument_rule.dart';

class ListenerSubscription implements ObservableInstrumentsSource, Disposable {
  final MeterListener _meterListener;
  final MetricsListener _metricsListener;
  final MeterFactory _meterFactory;
  Map<Instrument, Object?> _instruments = <Instrument, Object?>{};
  ...
  void _updateRules(List<InstrumentRule> rules) {
    ...
    tempListener.instrumentPublished =
        (instrument, _) => _refreshInstrument(instrument);
    // tempListener.start();
  }
  ...
  static bool _ruleMatches(
    ...
    if (meterName != null) {
      ...
      if (wildcardIndex >= 0 &&
        ...
        throw new InvalidOperationException(message: 'SR.MoreThanOneWildcard');
      }
