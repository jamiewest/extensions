...
extension PreferencesFlutterBuilderExtensions on FlutterBuilder {
  void addPreferences() {
    services
      ..addSingleton<PreferencesService>(
        (services) => PreferencesService(
          services
              ...
              .createLogger('Preferences'),
        ),
      )
      ..addHostedService<PreferencesService>(
        (services) => services.getRequiredService<PreferencesService>(),
      );
  }
}

class PreferencesService extends BackgroundService {
  ...
  Future<void> execute(CancellationToken stoppingToken) async {
    _logger.logTrace('PreferencesBackgroundService is starting...');
  ...
  String? _getAndDecryptValue(String unencryptedKey) {
    ...
    if (value == null) {
      ...
      try {
        ...
        _logger.logError('', exception: Exception()); //.severe(e, st);
        return null;
      }
    }
  }
